...
class Stocks(Resource):
    ...
    select_stock_max_date_query = ('SELECT date FROM stocks'
                                   ' ORDER BY date DESC LIMIT 1')
    select_stock_date_query = ('SELECT * FROM stocks'
    ...
    def patch(self, date):
        ...
        for item in html.xpath('/html/body/div/table/tbody/tr'):
            ...
            if (maxDate < itemDate):
                stockList.append(
                    (None, itemDate.strftime('%Y%m%d'), float(item[3].text),
    ...
    def get(self, date):
        ...
        for row in cursor.execute(self.select_stock_date_query, (date, )):
            stockList.append((row[0], row[1], row[2], row[3], row[4], row[5],
                              row[6], row[7], row[8]))
        conn.commit()
        conn.close()

...
class Funds(Resource):
    ...
    select_fund_max_date_query = 'SELECT date FROM funds ORDER BY date DESC LIMIT 1'
    select_fund_date_query = 'SELECT * FROM funds WHERE date >=? ORDER BY date DESC'
    ...
    def patch(self, date):
        ...
        for item in html.xpath('//*[contains(@class, "wfb2c") or contains(@class, "wfb5c")]/parent::tr'):
            ...
            if(len(item) == 4):
                ...
                if(maxDate < date):
                    fundList.append((None, date.strftime('%Y%m%d'), item[1].text))
        if(len(fundList) > 0):
